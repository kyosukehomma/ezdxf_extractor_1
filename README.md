...
現在は段階改修の第1段階です。土工表の検出と中心測点の選択は表単位になりましたが、
表内データの構築には従来の図面全体でのグループ化を再利用しています。
複数測点を含む横断図へ対応する次段階では、検出した罫線の範囲内だけから
種別・単位・数量を抽出します。
...
未対応ケースを誤出力せず停止できることも確認します。これらのファイルがない環境では、
該当するローカル回帰テストだけがスキップされます。
